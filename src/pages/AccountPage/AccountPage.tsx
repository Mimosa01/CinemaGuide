...
import { AccountFilmsSection } from "../../components/AccountFilmsSection/AccountFilmsSection";
...
export const AccountPage: FC<PropsAccount> = ({user}) => {
  const isMobile = window.innerWidth < 767 ? true : false;
  const [isFavorite, setIsFavorite] = useState(true);

  const onClick = (value: string) => {
    if (value === 'favorite' && isFavorite) return
    if (value === 'settings' && !isFavorite) return
    ...
    <section className='account'>
      <Container>
        <div className='account__container'>
          <h2 className='section-header account__header'>
            Мой аккаунт
          </h2>

          <nav className='account__nav account__nav--padding'>
            <Button view={{outline: true}} className='account__btn' onClick={() => onClick(FAVORITE)}>
            ...
            <Button view={{outline: true}} className='account__btn' onClick={() => onClick(SETTINGS)}>

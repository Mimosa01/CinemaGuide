import { openEntryModal } from "../../store/features/modalSlice";
...
import { useRegister } from "../../hooks/mutationHooks";

const RegisterShema = z.object({
  email: z.string().email('Введите корректный email'),
  name: z.string().min(1, 'Поле не ьщжут быть пустым'),
  surname: z.string().min(1, 'Поле не ьщжут быть пустым'),
  password: z.string().min(8, 'Поле должно содержать минимум 8 символов'),
...
type RegisterForm = z.infer<typeof RegisterShema>;
  ...
  return (
    <>
      <form className="modal__form" onSubmit={handleSubmit(onSubmit)}>
        <div className="modal__form-fields">
          ...
          <FormField errorMessage={errors.name?.message}>
            <input 
              ...
              {...register('name')}
            />
            <svg className="modal__icon" width="16" height="21" viewBox="0 0 16 21" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <FormField errorMessage={errors.passwordConfirm?.message}>
            <input 
              type="password" 
              className="modal__input" 
        ...
        <Button view={{color: true}} className="modal__btn" isLoading={registerMutate.isPending}>Создать аккаунт</Button>
      </form>
      <Button view={{outline: true}} className="modal__btn--outline" onClick={handleOpenEntryModal}>У меня есть пароль</Button>
    </>
  )
}